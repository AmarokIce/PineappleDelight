...
public class InitBlocks {
  ...
  private static Block registry(Block block, String name) {
    Registry.register(BuiltInRegistries.BLOCK,
        ResourceLocation.fromNamespaceAndPath("pineapple_delight", name), block);
  ...
  private static Block pie() {
    return registry(new PieBlock(
        Properties.ofFullCopy(ModBlocks.APPLE_PIE.get()),
  ...
  private static Block hugePineapple() {
    return registry(new Block(Properties.ofFullCopy(Blocks.MELON)),
        "huge_pineapple");
  }

  private static Block pineappleCrate() {
    return registry(new Block(Properties.ofFullCopy(Blocks.OAK_WOOD)),
        "pineapple_crate");
  }
}
