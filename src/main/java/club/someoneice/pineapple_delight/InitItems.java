...
public class InitItems {
  ...
  private static Item registry(Item item, String name) {
    Registry.register(BuiltInRegistries.ITEM,
        ResourceLocation.fromNamespaceAndPath("pineapple_delight", name), item);
    ITEMS.add(item);
    return item;
  }

  @SafeVarargs
  private static Item itemFoodDrink(String name, Holder<MobEffect>... effects) {
    var builder = new FoodProperties.Builder();
  ...
  private static class ItemDrink extends Item {
    ...
    Holder<MobEffect>[] effects = null;

    public ItemDrink(Item.Properties settings) {
      super(settings);
    }
    ...
    @SafeVarargs
    public final ItemDrink setEffect(Holder<MobEffect>... effect) {
      this.effects = effect;
      return this;
    }
    ...
    public ItemStack finishUsingItem(ItemStack stack, Level world, LivingEntity user) {
      ...
      if (effects != null) for (Holder<MobEffect> effect : effects)
